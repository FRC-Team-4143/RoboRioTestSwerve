...
private:
    std::map<std::string, RAWCConstant> data;
	static RAWCConstants * singletonInstance;
	
	RAWCConstants();
	//TODO: Add functionality to let this map persist to a file
	
	void saveDataToFile(std::string fileName); // Save to another file
	

	void restoreDataFromFile(std::string fileName);
...
public:
	...
	void insertKeyAndValue(std::string key, RAWCConstant value);
	...
	RAWCConstant getValueForKey(std::string key);
