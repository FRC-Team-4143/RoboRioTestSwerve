...
bool RAWCConstants::doesKeyExist(std::string key)
{
...
/// @param key
/// @return Stored Value
RAWCConstants::RAWCConstant RAWCConstants::getValueForKey(std::string key)
{
	if( doesKeyExist(key) )
	{
		return data[key];
...
void RAWCConstants::insertKeyAndValue(std::string key, RAWCConstant value)
{
	data[key] = value;
}
...
void RAWCConstants::saveDataToFile(std::string fileName)
{
	//Priv_SetWriteFileAllowed(1);
	std::map<std::string, RAWCConstant>::iterator it; //Iterator for map
...
void RAWCConstants::restoreDataFromFile(std::string fileName)
{
    std::string key;
    std::string valueString;
	
    std::ifstream infile ( RAWC_CONSTANTS_DEFAULT_FILE );
	printf("In restore\n ");
	if (infile.is_open())
	{	
		printf("File is open\n");
